...
& <skill>\scripts\eim-windows.ps1 RunIdf -EimPath <eim.exe> -CommandString 'idf.py --version' -IdfPath <IDF路径>
```

helper 以 argv 调用、复验签名并保留退出码;不要求新手为运行 Skill 先安装 Git Bash。
